 ...
 *
 */
package org.luceehibernate.intercept;

import org.luceehibernate.engine.SessionImplementor;
import org.luceehibernate.intercept.cglib.CGLIBHelper;
import org.luceehibernate.intercept.javassist.JavassistHelper;
...
public class FieldInterceptionHelper {
	...
	public static boolean isInstrumented(Class entityClass) {
		...
		for ( int i = 0; i < definedInterfaces.length; i++ ) {
			if ( "net.sf.cglib.transform.impl.InterceptFieldEnabled".equals( definedInterfaces[i].getName() )
			     || "org.luceehibernate.bytecode.javassist.FieldHandled".equals( definedInterfaces[i].getName() ) ) {
				return true;
			}
		}
		return false;
	...
	public static FieldInterceptor extractFieldInterceptor(Object entity) {
		...
		for ( int i = 0; i < definedInterfaces.length; i++ ) {
			...
			else if ( "org.luceehibernate.bytecode.javassist.FieldHandled".equals( definedInterfaces[i].getName() ) ) {
